...
class PlanarGripper : public systems::Diagram<double> {
 public:
  ...
  /// Sets up the diagram using the planar brick with texture.
  void SetupPlanarBrickWithTexture(std::string orientation);
 ...
 private:
  ...
  /// Adds a RGBD camera to the diagram.
  /// @param query_output_port The query output port from a SceneGraph.
  /// @param camera_name The name of the camera. Three output ports will be
  /// created based on the given name. They are {camera_name}_rgb_image,
  /// {camera_name}_depth_image, {camera_name}_label_image.
